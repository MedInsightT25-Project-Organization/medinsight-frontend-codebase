...
const DashboardCards = ({ scheduled, pending, cancelled }) => {
    const cards = [
        {
        ...
        <div className="overflow-x-auto md:overflow-visible">
            <div className="flex md:grid md:grid-cols-3 gap-6 p-6 min-w-[600px] md:min-w-0">
                {cards.map((card, index) => (
                    <div
                        key={index}
                        className={`min-w-[250px] md:min-w-0 rounded-xl card-shadow p-5 flex flex-col gap-4 transition-all duration-300 cursor-pointer`}
                    >
